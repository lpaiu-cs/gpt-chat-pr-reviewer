...
기존 프로젝트를 여세요. 터미널 안내에 따라 프로젝트 URL을 입력하면 접근을 확인하고
저장합니다. 로그인 세션과 프로젝트는 이후에도 재사용되며, 자동 리뷰 대화는 그 안에 모입니다.

프로젝트 URL을 이미 알고 있으면 `npm run dev -- setup --project-url <URL>`로 지정할 수 있습니다.
프로젝트가 없는 기존 설정도 `setup`을 한 번 실행해야 실제 리뷰를 시작할 수 있습니다.
