...
### 1. ChatGPT 로그인 및 리뷰 전용 프로젝트 설정

```bash
npm run dev -- setup
```

열린 Chrome에서 직접 로그인한 뒤, 사이드바에서 **리뷰 전용 프로젝트**를 만들거나
기존 프로젝트를 여세요. 프로젝트를 사이드바에 고정하고 터미널에서 Enter를 누르면
이름과 URL을 읽어 자동 진입을 검증한 뒤 저장합니다. 로그인 세션과 프로젝트는 이후에도 재사용됩니다.

비대화형 등록은 `npm run dev -- setup --project-url <URL> --project-name "프로젝트 이름"`을 사용합니다.
URL만 저장된 구버전 설정은 `setup`으로 이름도 등록하세요. 일반 Chrome과 자동화 Chrome은 계정이 다를 수 있습니다.
실제 리뷰 전 `npm run dev -- project-check --repeat 3`으로 진입만 검증할 수 있습니다. 실행 중인 데몬은 먼저 정상 종료하세요.

자동 실행은 창 없는 Chrome이 기본이고, setup 때만 브라우저 창을 표시합니다.
구버전 설정에 `headless: false`가 남아 있으면 `true`로 바꾸어야 기동 시에도 포커스를 뺏지 않습니다.
...
| **계정 변경** | 현재 리뷰가 끝나면 로그인 창을 열고, 새 계정의 리뷰 프로젝트 이름·URL까지 확인해 저장 |
