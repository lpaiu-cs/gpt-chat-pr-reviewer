...
기존 대화를 자동 이동하거나 삭제하지는 않습니다. 새 설정은 이후 생성·전송에 적용됩니다. 프로젝트 URL이 없거나 빈 문자열이면 실제 리뷰는 실행하지 않고 setup을 안내합니다. 관측 전용 `serve --observe`와 설정 조회는 프로젝트 없이 사용할 수 있습니다. 자동화 Chrome 프로필의 계정에도 해당 프로젝트 접근 권한이 있어야 합니다.
...
serve의 `ready=true`는 로그인뿐 아니라 프로젝트 자동 진입도 통과한 뒤에만 표시됩니다.
