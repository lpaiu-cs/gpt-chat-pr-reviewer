...
**그래서 대시보드는 "요청한 값" 을 따로 들고 있어야 한다** (app.html 의 `wanted`).
스냅샷의 `control` 은 **적용된** 값만 싣는데 컨트롤을 그것만으로 그리면, 3초마다
오는 스트리밍 갱신이 방금 누른 값을 곧바로 되돌려 놓는다 — 라운드가 도는 동안은
그 되돌림이 2~15분 내내 이어져 "눌러도 안 먹는다" 가 된다. 실제로 그렇게 동시
리뷰 수 변경이 고장 신고로 올라왔다 (백엔드는 멀쩡했고 화면만 지우고 있었다).
요청값은 `.pending` 점선으로 **적용 전임을 밝힌 채** 표시하고, 큐가 비고 보낸
요청도 다 응답받으면 놓는다 — 루프가 거절한 요청이 화면에 남지 않게 하는
안전장치다.
...
수준에서 막고, 브라우저(로그인 프로필)가 하나뿐이며, 라운드가 2~15분이라 세션이
동기적으로 기다릴 수도 없다. **직렬화 지점은 데몬 하나**이고 세션들은 아무것도
다투지 않는다. 동시 리뷰(`maxConcurrentReviews`)는 그 데몬 **안에서** 탭을 나눠
쓰는 것이지, 데몬을 여럿 띄우는 것이 아니다.
...
3. 스캔이 끝나면 `buildQueue` 로 우선순위를 매기고 앞에서 `maxConcurrentReviews`
   ...
   따로다(`ChatGPTDriver.fork`). 스캔과 라운드는 절대 겹치지 않는다 — 스캔이
   PRContext 를 다시 읽고 저장하기 때문이다 (f34146b 참고)
