...
**단 `--since-round` 가 있을 때만이다.** 조건 없이 현재 상태를 인정하면 반대
방향으로 틀린다: 1차가 `AWAITING_AUTHOR` 인 채로 수정을 push 하고 곧바로
기다리면, watch 가 새 커밋을 보기 전이라 첫 스냅샷은 여전히 `AWAITING_AUTHOR`
고 — 즉시 깨어나 2차를 아예 안 기다린다. 그래서 요청 시점의 라운드 번호를
기준으로 그 **이후** 결과만 인정하고(`daemon.mjs review` 가 알려준다), 기준이
없으면 재생하지 않는다. 놓치는 쪽이 오인하는 쪽보다 낫다 — 놓치면 다음 전이나
