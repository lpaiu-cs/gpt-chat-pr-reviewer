...
**첫 스캔 전에는 "범위 밖" 을 단정하지 않는다.** UI 는 브라우저 기동보다 먼저
열리므로(로그인 안내도 대시보드에 남아야 한다), `/api/state` 가 응답하는 시점에는
아직 아무 PR 도 안 보인다. 그 상태를 범위 밖으로 읽으면 멀쩡히 설정에 들어 있는
PR 에 대고 "범위를 넓히세요" 라고 답한다 — 하필 스킬이 넓힐 수 없게 만들어둔
바로 그 방향이라 사용자가 손으로 고치게 된다. `cycle.lastScanAt` 이 채워질 때까지
기다린 뒤 판정한다.
