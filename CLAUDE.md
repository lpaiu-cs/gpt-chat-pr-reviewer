...
- **이슈 #1** — 응답이 멈춰도 중지 버튼이 남아 `isStreaming()` 이 참을 유지하면
  연결 중단 복구(`!streaming` 조건)에 진입하지 못하고 타임아웃까지 대기한다.
  정체 타임아웃만 넣으면 오래 걸리는 추론을 절단하므로, **네트워크가 죽었다는
  적극적 근거**가 있을 때만 판정을 뒤집는다 (`judgeStreaming`). 생성 요청
  (`POST /backend-api/…conversation`)의 in-flight 수와 마지막 움직임을 추적해,
  버튼이 남아도 3분간 조용하면 스트림 종료로 본다. **한 번도 관측되지 않았으면
  (셀렉터·경로 불일치) 판정을 바꾸지 않는다** — 근거 없는 조기 절단이 더 나쁘다.
  화면이 멎으면 중지 버튼 DOM 을 자동으로 덤프해 (a)스트림 사망 / (b)셀렉터 오탐 /
  (c)실제 생성 중을 사후에 가릴 수 있게 남긴다
