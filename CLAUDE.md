...
잠금은 `src/lock.ts` 가 담당한다. **디렉터리**(`data/watch.lock/`)를 잠금으로 쓰는데,
어려운 부분이 생성이 아니라 **잔여 잠금 인수**이기 때문이다. 파일 + `wx` 로는 생성만
원자화되고, 인수는 `unlink(경로)` 라 "내가 본 그 잠금" 인지 확인할 수 없다 — A 가 인수를
마친 직후 B 가 A 의 멀쩡한 잠금을 지워 둘 다 돌게 된다. 디렉터리는 `rename` 이 그걸
푼다: 같은 디렉터리를 여럿이 옮기려 하면 하나만 성공한다.

`owner.json` 을 아직 못 읽는 잠금은 **뺏지 않는다**(`settling`) — mkdir 과 쓰기 사이의
틈에서 읽힌 것일 수 있다. 유예(10초)가 지나면 잔여물로 본다.

**UI 서버를 띄우기 전에** 잡는다 — 중복이 포트
