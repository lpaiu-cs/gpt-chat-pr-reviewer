...
`scope-add` / `scope-remove` 가 필요한 이유는 다중 세션 그 자체다. `scope-set` 은
read-modify-write 를 **클라이언트가** 하게 만드는데, 세션 A 가 자기 레포를 넣겠다고
전체 목록을 보내면 그 사이 B·C 가 넣은 레포가 사라진다. "무엇을 원하는가" 만
보내게 하고 병합은 루프가 한다. 추가는 캐시를 통째로 버리지 않고 `lastAt = 0`
으로 freshness 만 무효화한다 (제거는 `reset()` — 안 그러면 뺀 레포가 살아남아
리뷰를 게시할 수 있다).
...
`REVIEW_DUE` PR 을 자동 리뷰한다. `pr-watch` 의 `track`/`status`/`wait` 가 데몬을
...
**첫 기동은 씨앗이 필요하다.** `watch.include` 가 비면 `resolveWatchScope` 가 null
을 돌려주고 **UI 를 열기도 전에** 종료하므로, 붙어서 `scope-add` 를 넣으려던
클라이언트는 영영 문을 못 찾는다. 그래서 `watch --include <패턴>` 으로 기동 시점에
범위를 심는다 (메모리만 — 영속화는 뒤따르는 `scope-add` 가 정식 경로로 한다).
...
영영 안 온다 (기본 45분 타임아웃까지 기다린다). 계속 구독하는 모드에서는 하지
않는다 — 거기서 "지금 상태" 를 쏟으면 재연결마다 같은 소식이 반복된다.
