...
있고, 사람이 최종 판단을 내릴 수 있다. 스레드 카운트가 `n/n` 에 못 미친 채
남는 것이 "아직 열린 이견이 있다" 는 정확한 표시다.

**단, 코드 변경이 하나도 없으면 다음 라운드를 직접 요청해야 한다.**
`AWAITING_AUTHOR` 를 벗어나는 조건은 **새 커밋** 또는 **마지막 라운드 스레드
전체 resolve** 둘뿐이다. 지적이 전부 오탐이라 판단해 커밋이 없고 스레드도
열어 두면 두 조건이 **모두 거짓**이 되어 PR 이 그 상태에 머물고, 내 반론을
실은 다음 라운드가 아예 시작되지 않는다. 답변을 남긴 뒤 이렇게 요청한다:
