...
**이 스킬은 데몬을 띄우지 않는다.** 데몬을 새로 시작하면 브라우저가 뜨고 범위
안의 PR 이 자동으로 리뷰되기 시작한다 — 상태만 보려던 요청이 ChatGPT 한도를
쓰고 GitHub 에 리뷰를 게시하는 건 요청한 적 없는 부작용이다.

데몬이 돌고 있지 않으면 명령이 그 사실과 시작 방법을 알려준다. 그대로
사용자에게 전달하고, **임의로 시작하지 않는다.** 리뷰까지 원한다는 게 분명하면
`pr-review` 스킬을 쓴다.

`track` 은 레포 단위로 범위를 넓힌다 — 그 레포의 **다른 열린 PR 도** 리뷰
대상이 될 수 있다(필터 설정에 따름). 명령 출력이 이걸 경고하므로 사용자에게
함께 전달한다.
