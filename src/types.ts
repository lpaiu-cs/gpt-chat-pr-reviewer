...
export interface PRContext {
  ...
  /**
   * 마지막 게시 시점에 **아직 열려 있던** 스레드 id 들 — 작성자 응답의 판정 기준.
   *
   * 이것들이 전부 resolve 되면 작성자가 응답한 것이다. 게시 **시점**을 못 박는
   * 것이 핵심이다: 그때 이미 resolve 되어 있던 스레드는 이번 리뷰에 대한 응답이
   * 아니므로 근거가 될 수 없다.
   *
   * 이 스냅샷이 없던 시절, 판정이 "스레드가 있는 가장 최근 라운드" 로 물러섰다.
   * 그래서 인라인이 하나도 안 달린 라운드(모든 지적이 리뷰 본문으로 간 경우)
   * 뒤에는 **예전 라운드의 이미 resolve 된 스레드**가 판정 대상이 되어, 게시
   * 직후 곧바로 "전체 resolve" 가 성립했다. 실측: `sleep-management#3` 이
   * 8~13차에서 게시 6~11초 만에 재리뷰로 돌아가며 무한히 돌았다.
   *
   * 빈 배열은 "기다릴 것이 없었다" 는 뜻이고, 그때는 resolve 신호를 쓰지 않는다
   * (새 커밋만이 응답이다). 없으면(구버전 컨텍스트) 마지막 라운드 스레드로
   * 근사한다.
   */
...
export interface AppConfig {
  ...
  /**
   * 한 사이클에서 **동시에** 돌릴 리뷰 라운드 수. `0` 이면 제한 없음(대기열 전체).
   *
   * 기본 1 — ChatGPT 한도(Plus)를 아껴 쓰는 순차 처리다. 올리면 대기열 앞쪽을
   * 한꺼번에 돌린다. 라운드마다 브라우저 탭을 따로 쓰므로 한 탭의 입력창에 두
   * 프롬프트가 섞이는 일은 없지만, 같은 계정의 한도를 그만큼 빨리 쓴다.
   *
   * **동시 실행은 사이클 안에서만이다.** 스캔은 라운드가 하나도 돌지 않을 때만
   * 도는 규칙을 그대로 둔다 — 스캔은 PRContext 를 다시 읽고 저장하므로, 라운드와
   * 겹치면 진행 중인 라운드의 상태를 덮어쓴다.
   */
